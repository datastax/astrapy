...
class TestCollectionGeneralBM25Async:
    @pytest.mark.skipif(IS_ASTRA_DB, reason="Feature not on Astra DB yet")
    @pytest.mark.describe("test of collection bm25 DML with vectorize, async")
    async def test_collection_bm25_dml_vectorize_async(
        self,
        ...
        fol_doc = await acoll.find_one({"$lexical": {"$match": "water"}}, projection=PJ)
        assert fol_doc is not None
        assert fol_doc["_id"] == "lx"

        fcl_docs = await acoll.find(
            {"$lexical": {"$match": "water"}}, projection=PJ
        ).to_list()
        assert fcl_docs == [{"_id": "lx"}]

        # no lexical matching occurs
        fol_doc_n = await acoll.find_one(
            {"$lexical": {"$match": "fire"}}, projection=PJ
        )
        assert fol_doc_n is None

        # cannot find a doc without vector
        fov_doc_n = await acoll.find_one(sort={"$vectorize": "water"}, projection=PJ)
        ...
        upd_one_result = await acoll.update_one(
            {"$lexical": {"$match": "birds"}},
            {"$set": {"updated": True}},
            sort={"$lexical": "fly"},
        )
        ...
        upd_one_result_n = await acoll.update_one(
            {"$lexical": {"$match": "dogs"}},
            {"$set": {"updated": True}},
            sort={"$lexical": "fly"},
        )
        ...
        upd_one_result_n2 = await acoll.update_one(
            {"$lexical": {"$match": "birds"}},
            {"$set": {"updated": True}},
            sort={"$lexical": "cats"},
        )
        ...
        f1u_result_doc = await acoll.find_one_and_update(
            {"$lexical": {"$match": "birds"}},
            {"$set": {"updated_n": 1}},
            sort={"$lexical": "fly"},
            projection=PJ,
        ...
        u1_result = await acoll.update_one(
            {"$lexical": {"$match": "birds"}},
            {"$set": {"updated_n": 2}},
            sort={"$lexical": "fly"},
        )
        ...
        f1r_result_doc = await acoll.find_one_and_replace(
            {"$lexical": {"$match": "birds"}},
            {"name": "replaced 2"},
            sort={"$lexical": "fly"},
            projection=PJ,
        ...
        # r1_result = await acoll.replace_one(
        #     {"$lexical": {"$match": "birds"}},
        #     {"name": "replaced 1"},
        #     sort={"$lexical": "fly"},
        # )
        ...
        f1d_result_doc = await acoll.find_one_and_delete(
            {"$lexical": {"$match": "ants"}},
            sort={"$lexical": "tiptap"},
            projection=PJ,
        )
        ...
        # Suspended until #2150 is understood/resolved
        # f1d_result_doc_n = await acoll.find_one_and_delete(
        #     {"$lexical": {"$match": "ants"}},
        #     sort={"$lexical": "tiptap"},
        #     projection=PJ,
        # )
        ...
        d1_result = await acoll.delete_one(
            {"$lexical": {"$match": "worms"}},
            sort={"$lexical": "inch"},
        )
        ...
        # Suspended until #2150 is understood/resolved
        # d1_result_n = await acoll.delete_one(
        #     {"$lexical": {"$match": "worms"}},
        #     sort={"$lexical": "inch"},
        # )
        ...
        um_result = await acoll.update_many(
            {"$lexical": {"$match": "king"}},
            {"$set": {"updated": True}},
        )
        assert um_result.update_info["n"] == 2
        assert await acoll.count_documents({"updated": True}, upper_bound=5) == 2

        dm_result = await acoll.delete_many(
            {"$lexical": {"$match": "king"}},
        )
        assert dm_result.deleted_count == 2
        assert (
            await acoll.count_documents(
                {"$lexical": {"$match": "king"}},
                upper_bound=5,
            )
            == 0
        ...
        dm_result_n = await acoll.delete_many(
            {"$lexical": {"$match": "king"}},
        )
