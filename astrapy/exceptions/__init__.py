...
def _min_labeled_timeout(*timeouts: tuple[int | None, str | None]) -> tuple[int, str | None]:
    _non_null = [to for to in timeouts if to[0] is not None]
    if _non_null:
...
def _select_singlereq_timeout_gm(
    *,
    ...
    """
    if all(iarg is None for iarg in (general_method_timeout_ms, request_timeout_ms, timeout_ms)):
...
def _select_singlereq_timeout_ca(
    *,
    ...
    of them, disregarding the options altogether.
    """
    if all(iarg is None for iarg in (collection_admin_timeout_ms, request_timeout_ms, timeout_ms)):
        ao_r = timeout_options.request_timeout_ms
...
def _select_singlereq_timeout_ta(
    *,
    ...
    of them, disregarding the options altogether.
    """
    if all(iarg is None for iarg in (table_admin_timeout_ms, request_timeout_ms, timeout_ms)):
        ao_r = timeout_options.request_timeout_ms
...
def _select_singlereq_timeout_da(
    *,
    ...
    of them, disregarding the options altogether.
    """
    if all(iarg is None for iarg in (database_admin_timeout_ms, request_timeout_ms, timeout_ms)):
        ao_r = timeout_options.request_timeout_ms
