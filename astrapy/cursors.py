...
class Cursor(BaseCursor):
    ...
    def _create_iterator(self) -> _LookAheadIterator:
        ...
        def _find_iterator() -> Iterator[DocumentType]:
            ...
            #
            resp_0 = self._collection._astra_db_collection.find(
                filter=self._filter,
            ...
            _response_setter_callback(resp_0)
            ...
            while next_page_state is not None:
                _options_n = {**_options, **{"pageState": next_page_state}}
                resp_n = self._collection._astra_db_collection.find(
                    filter=self._filter,
                    projection=pf_projection,
                    sort=pf_sort,
                    options=_options_n,
                ...
                _response_setter_callback(resp_n)
                # TODO error if missing fields
